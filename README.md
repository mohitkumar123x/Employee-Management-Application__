...
The frontend provides a user-friendly interface for managing employees and departments. It includes features for viewing, adding, editing, and deleting employees and departments. The app also includes visualizations for employee metrics such as growth over time and distribution by age range.
...
```bash
git clone https://github.com/mohitkumar123x/Employee-Management-Application.git
...
First, ensure you have [Gradle](https://gradle.org/) and [Java JDK](https://www.oracle.com/java/technologies/javase-jdk11-downloads.html) installed.

Next, install the project dependencies and build the application:
...
```bash
git clone https://github.com/mohitkumar123x/Employee-Management-Application.git
cd Employee-Management-Application/frontend
```
...
Additionally, you can access the image on **Docker Hub** **[here](https://hub.docker.com/repository/docker/mohitkumar123x/employee-management-app/).**
...
The project also includes a `Jenkinsfile` for automating the build and deployment process using Jenkins. You can create a Jenkins pipeline job and use the `Jenkinsfile` to build and deploy the application.
